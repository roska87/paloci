...
// Errors travel as i18n keys, not prose: the UI speaks more languages than
// this module does, and t() falls back to the key itself if one is missing.
      ...
      (err, stdout) => {
        if (err && !stdout) {
          const code = err.code === "ENOENT"
            ? "semCliMissing"
            : (err.killed ? "semTimeout" : "semFailed");
          return resolve({ error: code, results: [] });
        }
